...
| `description` | DNS record set description. | `string` | `null` | no |
...
- When `description` is omitted, the module generates one in the form `type='<type>' recordset='<name>' with ttl='<ttl>'`.
