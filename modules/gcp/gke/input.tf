...
variable "node_pools" {
  ...
  type = map(object({
    ...
    management          = optional(any, { auto_repair = true, auto_upgrade = true })
    node_config         = optional(any, {})
    upgrade_settings    = optional(any, { max_surge = 1, max_unavailable = 0 })
    placement_policy    = optional(any)
    queued_provisioning = optional(any)
